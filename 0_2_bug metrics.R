...
tibble(FIELD=names(meta_arth_df)) %>% write_clip()
meta_arth_df %>%
  select(
    FinalID,
    # CommonName,
...
sample_data<-
  ...
  filter(str_detect(CollectionMethodName, "Invert")) %>% #Only bug data, not bryophytes
  filter(!FinalID %in% c("Anura","Gastropoda","Annelida", "Eucladium")) %>% #Not arthropods. Eucladium accidentally has wrong method
  #Field names from CEDEN don't match expectations
...
# setdiff(sample_data$FinalID, meta_arth_df2$FinalID)
# setdiff(meta_arth_df2$FinalID, meta_arth_df2$FinalID) #No difference between new and older version of STE
# setdiff(meta_arth_df2$FinalID, meta_arth_df2$FinalID) 
...
metrics_output<-sample_data_aggregated %>%
  ...
  summarise(
    ...
    #Hemiptera #ADDED June 6, 2024 
    Hemip_Rich=sum(DistinctTaxon & Order=="Hemiptera", na.rm = T),
    Hemip_RelRich=Hemip_Rich/Arth_Rich,
    Hemip_Abund = sum(Result[Order=="Hemiptera"], na.rm = T),
    Hemip_RelAbund=Hemip_Abund/Hemip_Abund,
    #Araneae
...
calculate_bug_metrics<-function(x) {
  x %>%
    ...
    summarise(
      ...
      Hemip_RelRich=Hemip_Rich/Arth_Rich,
      Hemip_Abund = sum(Result[Order=="Hemiptera"], na.rm = T),
      Hemip_RelAbund=Hemip_Abund/Arth_Abund,
      #Araneae
      Araneae_Rich=sum(DistinctTaxon & Order=="Araneae" , na.rm=T),
      Araneae_RelRich=Araneae_Rich/Arth_Rich,
      Araneae_Abund = sum(Result[Order=="Araneae"], na.rm=T),
...
arth_mets<-c("Arth_Rich", "Arth_Abund", 
            ...
            "Hemip_Rich", "Hemip_RelRich", "Hemip_Abund", "Hemip_RelAbund", 
...
junk<-
  ...
  pivot_longer(cols=all_of(arth_mets)) %>%
  mutate(value=case_when(value==Inf~NA_real_,
                         value==-Inf~NA_real_,
                         T~value)) 
# junk %>% filter(value==-Inf)

write_csv(junk, "Data/BioData/Metrics/arthropod_metrics_04242024.csv")
